...
public class CliParser {
    ...
    private final List<ParameterGroup> commandLineParameterGroups
        = new ArrayList<>();
    ...
    public void addParameterGroup(
        String groupName,
        List<? extends CliParameter> parameters
    ) {
        commandLineParameterGroups.add(new ParameterGroup(
    ...
    public void parseArguments(String[] args) throws CliParseException {
        ...
        if (commandLineParameterGroups.get(0).name.isEmpty()) {
            ...
            if (parameters == null) {
                throw new CliParseException("Group name "
                    + args[0]
                    + " is undefined.");
            }
            i++;
        ...
        for (CliParameter option : parameters) {
            if (!option.isOptional() && !option.isSelected()) {
                throw new CliParseException("Parameter "
                    + option.getName()
                    + " is mandatory but was not selected.");
            }
        }
    }
    ...
    public String getUsageString() {
        ...
        for (ParameterGroup parameterGroup : commandLineParameterGroups) {
            ...
            for (CliParameter parameter : parameterGroup.parameters) {
                if ((characterColumn + parameter.appendSynopsis(new StringBuilder()))
                    > 90) {
                    characterColumn = 0;
                    sb.append("\n\t    ");
                }
